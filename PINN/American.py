...
class AmericanPINN(torch.nn.Module):
    ...
    def pde_nn(self, S, tau):
        ...
        ############################### To be optimized ###############################
        bool_arr = torch.tensor([self.type == 'put']*len(S))
        intrinsic = torch.where(bool_arr,
    ...
    def train(self, S_ib, tau_ib, V_ib, S_pde, tau_pde, S_data=None, tau_data=None, V_data=None,
              epochs=1000, optimizer='adam', verbose=True, loss_weights=[1, 1, 1], **kwargs):
        if optimizer == 'adam':
        ...
        for i in range(epochs):
            ...
            loss_ib, loss_pde, loss_data = self.loss(S_ib, tau_ib, V_ib, S_pde, tau_pde, S_data, tau_data, V_data)
            loss_ib, loss_pde, loss_data = loss_weights[0]*loss_ib, loss_weights[1]*loss_pde, loss_weights[2]*loss_data
